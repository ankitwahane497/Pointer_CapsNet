...
def train(model, supervisor, num_label, dataset_it,n_batch):
    ...
    with supervisor.managed_session(config=config) as sess:
        ...
        for epoch in range(cfg.epoch):
            ...
            if supervisor.should_stop():
                ...
                break
            for step in tqdm(range(n_batch), total=num_tr_batch, ncols=70, leave=False, unit='b'):

                if global_step % cfg.train_sum_freq == 0:
                    _, loss, train_acc, summary_str = sess.run([model.train_op, model.total_loss, model.accuracy, model.train_summary],
                     {model.X:data, model.labels: labels)
...
def main(_):
    ...
    dataset_it = dataset_iterator()

    sv = tf.train.Supervisor(graph=model.graph, logdir=cfg.logdir, save_model_secs=0)

    if cfg.is_training:
        tf.logging.info(' Start training...')
        train(model, sv, num_label, dataset_it)
