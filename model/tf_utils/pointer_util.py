...
def get_global_features(data,knn,k):
...
def get_local_features(data,knn,k):
    nn_pts = [np.repeat(data[i],k,axis =-2) - np.squeeze(data[i][knn[i]],axis =-2)  for i in range(len(data))]
